...
Move with WASD/arrows (mobile: floating touch joystick). Arrows auto-fire at the nearest enemy in line of sight — there is no fire button. Every enemy kills on touch; one hit ends the run. Dash (Shift/A) blinks you backward; Burst (Space/B) doubles your fire rate briefly. Each run generates a fresh grid of rooms: traverse it, survive the waves, reach the boss arena.
