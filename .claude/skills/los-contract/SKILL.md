...
- `src/game/Game.ts`
  - `findNearestVisibleEnemy()` (`Game.ts:1452`) — scans all enemies and returns the nearest by Euclidean center distance (centers = position + `TILE_SIZE/2`), skipping any beyond `maxDetectionRange` (a `Game` field initialized from `MAX_DETECTION_RANGE` = 450, `constants.ts`) or without LOS. Single source of truth for both auto-fire targeting **and** the on-screen LOS indicator.
  - `hasDirectLineOfSight(start, end)` (`Game.ts:1486`) — generic point-to-point raycast, sampled in equal steps at ~8 px granularity (`steps = ceil(distance / 8)`); blocked if any sampled tile is a wall; distance 0 returns `true`. Walls are the only gate — any angle qualifies. Center-to-center is intentional: an enemy peeking out from behind a wall column doesn't qualify until the player steps to clear the line.
  - `fireArrow(target)` (`Game.ts:1516`) — emits an arrow on the raw unit vector `{dx/d, dy/d}` toward the target center (no angle snapping); early-returns on distance 0. The per-frame cooldown check in `update` uses `ARROW_COOLDOWN_MS / stamina.getArrowRateMultiplier()` (burst × low-stamina). Arrows fly at `ARROW_SPEED` (400 px/s) and die on canvas bounds, walls, or enemy hit.
  - `checkCollisions()` (`Game.ts:1543`) — pass 1 kills the player on center-delta overlap (`|dx| < TILE_SIZE/2 && |dy| < TILE_SIZE/2`) against the 32 px cell — **not** the per-type `ENEMY_AABB_PX` footprints, which govern wall and enemy-vs-enemy collision only. Pass 2 checks the 4×4 arrow box against the 32 px enemy cell; a hit removes the enemy and scores +10.
...
5. **Player health, regen, armor, i-frames.** One hit = death. CLAUDE.md §5.
