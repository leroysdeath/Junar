...
description: The React ↔ game-state seam in Junar. UI state lives in React, game state lives in plain TS classes in src/game/, and the only bridge is the GameCallbacks object passed to Game's constructor. Use when adding HUD overlays, wiring new game→UI signals, or whenever a request would move game state into React, pull React/hooks into src/game/, or introduce a state library (Redux/Zustand/Jotai/MobX/Recoil/etc.).
---
