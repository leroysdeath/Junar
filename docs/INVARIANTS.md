...
**Verify:**
- `grep -rEn "import.*from.*assets|\\.png|\\.jpg|\\.webp" src/game/` returns imports (plus comments) only in `Renderer.ts`, and only for: `player-sprite.png`, `sprites/family-*.png`, and `sprites/{panther,bear,snake,gibbon}.png`.
- `ctx.drawImage` call sites exist only in `Renderer.renderPlayer`, `Renderer.renderNpcs`, and `Renderer.drawBeast` (the shared helper behind `renderPanther` / `renderBear` / `renderSnake` / `renderGibbon`).
