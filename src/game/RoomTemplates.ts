...
// T6 — Dead-end chamber (N only; chamber holds a static cache). The four
// dead-end rotations (N/S/E/W) are the "teardrop rooms" that hold mangos
// (owner 2026-06-19): a bulb chamber + a single corridor stem. The chamber
// CENTER is kept as plain floor (not an 's'/'S' static candidate) so the
// mango — placed there by Game at MANGO_TILE_BY_TEMPLATE — owns that tile and
// statics only roll on the corner candidates.
...
// Dead-end chamber, S stem (vertical flip of T6) — chamber at top, S opening.
// Mango tile: chamber center (col 14, row 7).
const T_DEADEND_S: ConnectorSource = {
  id: 't-deadend-s',
  ascii: [
    '#############################',
    '#############################',
...
// Dead-end chamber, E stem — chamber on the left, E opening (rows 7–9).
// Mango tile: chamber center (col 8, row 8).
const T_DEADEND_E: ConnectorSource = {
  id: 't-deadend-e',
  ascii: [
    '#############################',
    '#############################',
...
// Dead-end chamber, W stem — chamber on the right, W opening (rows 7–9).
// Mango tile: chamber center (col 20, row 8).
const T_DEADEND_W: ConnectorSource = {
  id: 't-deadend-w',
  ascii: [
    '#############################',
    '#############################',
...
// Fork-S / Fork-W — the remaining two fork (comb) rotations, completing the set
// (owner 2026-06-20). Fork-S: three S openings (5-7/13-15/21-23) + a single N
// stem. Fork-W: three W openings (rows 3-5/7-9/11-13) off a spine + a single E.
const T_MULTIOPEN_FORK_S: ConnectorSource = {
  id: 't-multiopen-fork-s',
  ascii: [
    '#############...#############',
    '#############...#############',
    '#############...#############',
...
const T_MULTIOPEN_FORK_W: ConnectorSource = {
  id: 't-multiopen-fork-w',
  ascii: [
    '#############################',
    '#############################',
    '#############################',
...
// ───────────────────────────────────────────────────────────────────────────
// Mini-boss link adapters (owner 2026-06-20). Connectors whose openings match a
// specific mini-boss room's NON-canonical openings on one edge and bridge to a
// canonical opening on another, so the corridor fabric flows cleanly into the
// snake / gibbon arenas (anchorAgreement biases these toward those rooms). The
// bear arena uses canonical doorways and needs no adapter; the snake arena's S
// openings (5-7/21-23) are already mated by t-multiopen-double-n.
// ───────────────────────────────────────────────────────────────────────────

// Snake-link N — S openings 6-8/20-22 (mate snake's N) bridge to a canonical N.
const T_SNAKE_LINK_N: ConnectorSource = {
  id: 't-snake-link-n',
  ascii: [
    '#############...#############',
    '#############...#############',
...
// Snake-link E — W openings rows 1-3/9-11 (mate snake's E) bridge to canonical E.
const T_SNAKE_LINK_E: ConnectorSource = {
  id: 't-snake-link-e',
  ascii: [
    '#############################',
    '....#########################',
...
// Snake-link W — E openings rows 1-3/9-11 (mate snake's W) bridge to canonical W.
const T_SNAKE_LINK_W: ConnectorSource = {
  id: 't-snake-link-w',
  ascii: [
    '#############################',
    '#########################....',
...
// Gibbon-link S — N openings 13-15/25-27 (mate gibbon's S) bridge to a canonical S.
const T_GIBBON_LINK_S: ConnectorSource = {
  id: 't-gibbon-link-s',
  ascii: [
    '#############...#########...#',
    '#############...#########...#',
...
// (Cross-pillars chamber moved to the panther mini-boss "Empty Throne" arena in
// RoomGrid.ts on 2026-06-20 — removed from the connector pool.)
