...
export class Level {
  ...
  findSafeSpawnPosition(
    ...
    for (const fallback of fallbackPositions) {
      if (this.isPositionSafe(fallback.x, fallback.y, playerSize, playerSize)) {
        console.warn(
          `Using fallback spawn position at (${fallback.x}, ${fallback.y}) - original position (${intendedPosition.x}, ${intendedPosition.y}) was unsafe`,
        );
        return { ...fallback };
      }
    }
    ...
    console.error(
      `Could not find safe spawn position! Using intended position (${intendedPosition.x}, ${intendedPosition.y}) - player may spawn in wall`,
    );
    return { ...intendedPosition };
  }
  ...
  generateEdgeEnemySpawns(
    enemyCount: number,
    minDistanceFromPlayer: number = 128,
  ): EnemySpawn[] {
