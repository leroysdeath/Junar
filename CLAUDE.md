...
- **Readable at a glance.** Every entity must be identifiable in one frame at 32×32. **Characters and beasts are sprite-based** (owner-greenlit: player 2026-05-10; the four beasts and the family 2026-06-11 — Tiers 1/2 of `docs/ART-ASSETS.md`); **environment, hut, arrows, FX, HUD, and the plant boss stay procedural** (Tier 3 not greenlit; Tier 4 keep-procedural confirmed 2026-06-11). Sprite or rectangle, readability is the bar — a sprite that muddies the silhouette is worse than the rectangles it replaced.
...
**Family** — wife, son (boy), daughter. They lived in harmony with the jungle and read it like a second language. They are the player's reason. Rendered as translucent 16×18 sprites (recolored from Charles Gabriel/Antifarea's CC-BY 3.0 charsets — see `docs/ART-CREDITS.md`): deep warm-brown skin and black hair matching the player's palette, wife in a cream apron over a red dress, son in a red/cream-striped tunic, daughter in a turmeric-ochre top — dignified, minimal, no headdress or caricature. They currently appear in anchors 6–9 (the levels whose ASCII grids carry `N` markers) as passive render-only figures (translucency = "not yet interactive") and will gain AI and death triggers once family mechanics are implemented.
...
**Bestiary status:**
- **Approved and implemented:** Black panther (Indian leopard), sloth bear, Indian rat snake, Hoolock gibbon. Speed tuned so panther/bear outrun the player while snake/gibbon are slower — combat depends on chokepoints, not foot races. All four currently render procedurally; the sprite swap is owner-greenlit (Tier 1, 2026-06-11) and sourced from the paid Time Fantasy Animals packs (purchase pending — see `docs/ART-ASSETS.md` Tier 1 status), with the red-eye cue staying a procedural overlay on top.
...
**Rendering:**
- **Sprite (characters + beasts; `docs/ART-ASSETS.md` Tiers 1–2, greenlit 2026-06-11):** player (CC0 LTTP pack, 16×32 cells, 4-frame walks — in use since 2026-05-10); family wife/son/daughter (CC-BY 3.0 Antifarea recolors, 16×18 cells, 3-frame 4-dir walks — in use, idle frame only until the `FamilyMember` entity lands); the four beasts (Time Fantasy packs — greenlit and sourced, landing once the owner's purchase arrives; procedural bodies render meanwhile). All sprites draw via `ctx.drawImage` with `imageSmoothingEnabled = false`, bundled as PNGs through Vite.
- **Procedural (everything else — confirmed 2026-06-11):** walls/floor, huts, arrows, burst aura, LOS indicator, materialize flash, FX, and the corrupted growth / plant boss render as hand-drawn Canvas 2D fills in `Renderer.ts` (Tier 3 not greenlit; Tier 4 keep-procedural confirmed). The infected red-eye cue stays a procedural overlay even on sprite beasts. Canonical colours live in `Renderer.ts` — don't duplicate them here.
...
- Sprite assets beyond the approved set: player (2026-05-10), the four beasts and the family (Tiers 1–2 of `docs/ART-ASSETS.md`, greenlit 2026-06-11). Environment tiles/hut (Tier 3 — explicitly not greenlit), arrows, FX, HUD, and the plant boss (Tier 4 — keep-procedural confirmed) stay procedural; extending sprites to any of them needs fresh owner approval. License policy for sourced art: CC0 / CC-BY / paid royalty-free only — **share-alike is rejected** (no CC-BY-SA, no GPL, hence no LPC; resolved in `docs/IDEATION.md` §8), no NC/ND, nothing AI-generated.
...
5. **Family rendering & combat** — rendering half landed 2026-06-11 (sprite swap, idle frames at `N` markers); still to build: the `FamilyMember` entity (movement, collision, walk animation from the existing sheets, death-triggers-game-over, simple combat behavior for the boss fight). Beast sprite swap (Tier 1) lands here too once the Time Fantasy purchase arrives.
...
- **Sprite assets are limited to the approved set: player, the four beasts, and the family.** (Player owner-approved 2026-05-10; beasts + family greenlit 2026-06-11 as Tiers 1–2 of `docs/ART-ASSETS.md`.) Walls, floor, huts, HUD, arrows, FX, and the plant boss render procedurally via `Renderer.ts` — Tier 3 was explicitly not greenlit and Tier 4's keep-procedural recommendation was confirmed. Don't extend sprite assets to any other entity without owner approval. Sourced art must be CC0 / CC-BY (logged in `docs/ART-CREDITS.md`) / paid royalty-free; share-alike (CC-BY-SA, GPL — including all LPC assets) is rejected, as are NC/ND and anything AI-generated.
...
└── game/                         All game logic — pure TS, no React
    ...
    ├── Renderer.ts               Canvas 2D draw routines for the current room/player/enemies/arrows/NPCs/huts/corrupted growth (sprites for player + family — and beasts once Tier 1 lands — procedural for everything else)
